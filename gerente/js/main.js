// gerente/js/main.js
// Punto de entrada principal del panel gerente - VERSIÓN COMPLETA CON EDITOR CONTENTEDITABLE
...
async function renderizarTareas() {
    ...
    if (subvista === 'editar') {
        ...
        const { renderizarFormularioEditar } = tareasModule
        const tecnicos = await tareasModule.cargarTecnicos(currentEmpresaId)
        const clientes = await tareasModule.cargarClientesDeEmpresa(currentEmpresaId)
        const servicios = await tareasModule.cargarServicios()
        const tiposTarea = await tareasModule.cargarTiposTarea()
        ...
        return `<div class="container">
            <div class="card">
                <div class="card-header">✏️ Editar Tarea
                    <button id="btnVolverTareas" class="btn-warning" style="float:right;">◀ Volver</button>
                </div>
                ${renderizarFormularioEditar(tarea, clientes, tecnicos, servicios, tiposTarea, plantillas)}
            </div>
        </div>`
    }
...
// ============================================================
// RENDERIZAR DETALLE DE TAREA CON EDITOR DE DESCRIPCIÓN
// ============================================================
...
async function renderizarDetalleTarea(tarea) {
    ...
    if (historialEstados && historialEstados.length > 0) {
        historialEstados.forEach(h => {
            historialCombinado.push({
                fecha: h.fecha,
                fecha_hora: h.fecha_hora || h.fecha,
                tipo: 'estado',
                estado: h.estado_nuevo,
                comentario: h.comentario || '',
                usuario: h.usuario_tipo || 'sistema',
                badge: getEstadoBadgeClass(h.estado_nuevo),
                label: getEstadoLabel(h.estado_nuevo)
            });
        });
    }
    ...
    if (historial && historial.length > 0) {
        historial.forEach(h => {
            const nombreTecnico = h.tecnico?.nombre_razon_social || 'técnico';
            const motivo = h.motivo ? ` (${h.motivo})` : '';
            const accion = h.tipo === 'asignacion' ? 'Asignada' : 'Reasignada';
            historialCombinado.push({
                fecha: h.fecha_asignacion,
                fecha_hora: h.fecha_asignacion,
                tipo: 'asignacion',
                estado: null,
                comentario: `${accion} a ${nombreTecnico}${motivo}`,
                usuario: 'gerente',
                badge: 'badge-info',
                label: '🔄 Asignación'
            });
    ...
    const historialHTML = historialCombinado.length > 0 ? `
    <div class="card" style="margin-top:16px;">
        <div class="card-header">📜 Historial de cambios</div>
        ${historialCombinado.slice(0, 20).map(h => `
            <div style="padding:6px 0; border-bottom:1px solid var(--ios-border); font-size:13px;">
                ...
                <span style="color:#6b7280; font-size:11px;">(${h.usuario})</span>
            </div>
        `).join('')}
    </div>` : '';
...
// ============================================================
// FUNCIÓN PARA CAMBIAR ESTADO DESDE EL DETALLE
// ============================================================
...
async function guardarNuevaTarea() {
    ...
    // ✅ Enriquecer descripción con DeepSeek - CORREGIDO
    mostrarModalCarga('📝 Generando descripción profesional...')
    
    let descripcionFinal = descripcion || 'Tarea de mantenimiento'
    try {
        // ✅ Asegurar que servicio y tipo tengan valor
        const nombreServicio = servicio || 'Servicio general';
        const nombreTipo = tipo || 'Tarea general';
...
function asignarEventosSubmodulos() {
    ...
    // ✅ EDITOR DE DESCRIPCIÓN - Abrir formulario de edición
    document.getElementById('btnEditarDescripcion')?.addEventListener('click', function() {
        ...
        localStorage.setItem('gerente_tareas_subvista', 'editar');
        localStorage.setItem('gerente_tarea_editar', tareaId);
        renderizarPanel();
    });

    // ✅ GUARDAR EDICIÓN DE TAREA
    document.getElementById('btnGuardarEdicionTarea')?.addEventListener('click', async function() {
        ...
        const descripcionDiv = document.getElementById('editTareaDescripcion');
        let descripcion = '';
        if (descripcionDiv) {
            descripcion = descripcionDiv.innerHTML || '';
            if (!descripcion.trim() || descripcion === '<br>') {
                descripcion = descripcionDiv.textContent || '';
            }
        }
        ...
        const fechaLimite = document.getElementById('editTareaFechaLimite')?.value || null;
        const tiempoEstimado = parseInt(document.getElementById('editTareaTiempoEstimado')?.value) || null;
        ...
        const activoId = document.getElementById('editTareaActivo')?.value || null;
        const servicioId = document.getElementById('editTareaServicio')?.value;
        const tipoTareaId = document.getElementById('editTareaTipo')?.value;
        const plantillaId = document.getElementById('editTareaPlantilla')?.value || null;
        
        if (!titulo || !clienteId || !servicioId || !tipoTareaId) {
            mostrarMensaje('Completa los campos obligatorios (Título, Cliente, Servicio y Tipo)', 'error');
            return;
        }
        ...
        try {
            const { error } = await sb
                ...
                .update({
                    ...
                    fecha_fin_prevista: fechaLimite,
    ...
    // ✅ CANCELAR EDICIÓN
    document.getElementById('btnCancelarEdicionTarea')?.addEventListener('click', function() {
        localStorage.setItem('gerente_tareas_subvista', 'detalle');
        renderizarPanel();
    });

    // ✅ REGENERAR DESCRIPCIÓN CON IA EN EDICIÓN
    document.getElementById('btnRegenerarDescEditar')?.addEventListener('click', async function() {
        const tareaId = document.getElementById('editTareaId')?.value;
        if (!tareaId) {
            mostrarMensaje('Error: Tarea no encontrada', 'error');
            return;
        ...
        try {
            const nombreServicio = tarea.servicio?.nombre || 'Servicio general';
            const nombreTipo = tarea.tipo_tarea?.nombre || 'Tarea general';
            const enriquecida = await enriquecerDescripcion(
                nombreServicio,
                nombreTipo,
                tarea.descripcion || 'Tarea de mantenimiento',
                tarea.cliente?.nombre || 'Cliente',
                tarea.activos?.nombre || 'Activo'
            );
            ...
            if (enriquecida) {
                const descripcionDiv = document.getElementById('editTareaDescripcion');
                if (descripcionDiv) {
                    descripcionDiv.innerHTML = enriquecida;
    ...
    // ✅ FILTROS DINÁMICOS: Servicio → Tipos → Plantillas (CREAR)
    setTimeout(() => {
    ...
    setTimeout(() => {
        const editClienteSelect = document.getElementById('editTareaCliente');
        if (editClienteSelect) {
            const newEditClienteSelect = editClienteSelect.cloneNode(true);
            editClienteSelect.parentNode?.replaceChild(newEditClienteSelect, editClienteSelect);
            
            newEditClienteSelect.addEventListener('change', async function() {
                const clienteId = this.value;
                const activoSelect = document.getElementById('editTareaActivo');
                ...
                try {
                    ...
                    if (activos && activos.length > 0) {
                        const tareaId = document.getElementById('editTareaId')?.value;
                        const tarea = tareasData.find(t => t.id === tareaId);
                        const activoActual = tarea?.activo_id || '';
                        
                        activos.forEach(a => {
                            options += `<option value="${a.id}" ${a.id === activoActual ? 'selected' : ''}>${escapeHtml(a.nombre)}</option>`;
                        });
                    } else {
                        options += '<option value="" disabled>📭 Sin activos</option>';
                    }
